...
sealed class ControlStyle(val style: String) {
    ...
    data class Code(val selectedText: String?) : ControlStyle(
        style = "<div style=\"background-color:${getSitePalette().secondary};color:${getSitePalette().white};font-size:14px;padding:12px;border-radius:6px;\"><pre><code class=\"language-kotlin\">$selectedText</code></pre></div>"
    )
